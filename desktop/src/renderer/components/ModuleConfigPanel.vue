...
const CARDS = {
  ...
  tools: [
    ...
    { id:"it", title:"OpenClaw 集成", desc:"integration_local.json", fields:[F("integration.clawguard_url","Argus 地址","text"),F("integration.timeout_ms","超时(ms)","number",{min:1000}),F("integration.fail_mode","失败模式","select",{options:[{label:"closed",value:"closed"},{label:"open",value:"open"}]}),F("integration.enable_media_check","附件送检","bool"),F("integration.api_token_env","API令牌环境变量名","text"),F("integration.media_root","附件根目录","text"),F("integration.protected_tools","保护工具","tags")] },
  ],
  sandbox: [
