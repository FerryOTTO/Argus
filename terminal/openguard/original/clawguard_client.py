# clawguard_client.py — Clawguard 统一安全服务 HTTP 客户端（对接方案 10.5）
"""
调用 Clawguard 四个统一接口：
...
class ClawguardClient:
    ...
    def __init__(self, base_url: str = "", timeout: int = 0):
        self.base_url = base_url or config.clawguard_url
        self.timeout = timeout or config.clawguard_timeout
    ...
    async def _post(self, path: str, request: SecurityRequest) -> SecurityResponse:
        ...
        try:
            ...
            return SecurityResponse(
                ...
                risk_score=0.0,
                reason=f"clawguard_unavailable: {type(exc).__name__}",
                module_results=[],
            )
