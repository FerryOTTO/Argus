# test_clawguard_integration.py — OpenGuard <-> Clawguard Integration Tests
"""
测试范围：
  1. 身份头注入是否正确
  2. /health 格式是否符合对接方案 8.1
  3. trace_id 生成和传递
  4. Clawguard 客户端超时/异常安全

运行方式：
  pytest test_clawguard_integration.py -v
  或直接运行: python test_clawguard_integration.py
"""
import sys
import uuid
try:
...
from middleware import get_clawguard_headers
from config import config


class TestClawguardHeaders:
    """对接方案 7.7：X-Clawguard-* 身份头注入"""
    ...
    def test_headers_from_usercontext(self):
        ...
        headers = get_clawguard_headers(uc)

        assert headers["X-Clawguard-User-Id"] == "usr_abc123"
        assert headers["X-Clawguard-Session-Id"] == "sess_xyz789"
        assert headers["X-Clawguard-Trace-Id"] == "openguard-abcdef123456"
        assert headers["X-Clawguard-Role"] == "user"
        assert headers["X-Clawguard-Security-Level"] == "internal"
    ...
    def test_headers_from_dict(self):
        ...
        headers = get_clawguard_headers(payload)

        assert headers["X-Clawguard-User-Id"] == "usr_001"
        assert headers["X-Clawguard-Session-Id"] == "sess_002"
        assert headers["X-Clawguard-Role"] == "admin"
        assert headers["X-Clawguard-Trace-Id"] == "trace-999"

    def test_headers_default_values(self):
        """缺失字段时使用默认值"""
        minimal = {"sub": "usr_min"}
        headers = get_clawguard_headers(minimal)
        assert headers["X-Clawguard-User-Id"] == "usr_min"
        assert headers["X-Clawguard-Role"] == "user"
        assert headers["X-Clawguard-Security-Level"] == "internal"
        assert headers["X-Clawguard-Trace-Id"].startswith("openguard-")
...
class TestClawguardClient:
    """测试 clawguard_client.py 的容错行为"""

    async def _test_timeout_returns_allow(self):
        """超时时返回 allow，不抛异常"""
        from clawguard_client import ClawguardClient

        client = ClawguardClient(base_url="http://127.0.0.1:19999", timeout=1)
        rc = RequestContext(
...
if __name__ == "__main__":
    # 不依赖 pytest 的手动运行方式
    print("=" * 50)
    print("OpenGuard <-> Clawguard Integration Tests")
    print("=" * 50)

    tests = TestClawguardHeaders()
    ...
    client_tests = TestClawguardClient()
