...
test("middleware fails closed before an unchecked tool result reaches the model", async (t) => {
  ...
  assert.match(checked.result.content[0].text, /Clawguard 已隔离/);
});
...
test("fails closed when a protected tool result has no completed check", () => {
  ...
  assert.match(result.message.content[0].text, /Clawguard 已隔离/);
...
test("applies cached rewrite and block actions to persisted tool results", async (t) => {
  ...
  assert.match(blocked.message.content[0].text, /Clawguard 已隔离/);
});

test("input and output hooks enforce Clawguard decisions", async (t) => {
  const { handlers } = registerPlugin();
  const responses = [
...
test("binds original prompt and enforces tool pre-check actions", async (t) => {
  ...
  assert.equal(review.requireApproval.pluginId, "clawguard-adapter");
...
test("can auto-allow human review only for an explicitly listed demo tool", async (t) => {
  ...
  assert.ok(
    logs.some(
      ([level, message]) =>
        level === "warn" && message.includes("clawguard-human-review-auto-allowed"),
    ),
  );
  ...
  assert.equal(execReview.requireApproval.pluginId, "clawguard-adapter");
});
