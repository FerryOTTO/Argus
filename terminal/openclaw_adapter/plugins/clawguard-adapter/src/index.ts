...
const BLOCKED_CONTEXT_MESSAGE =
  "[Clawguard 已隔离此工具结果：内容安全检查未通过。]";
const BLOCKED_TOOL_MESSAGE = "工具调用触发了安全策略，本次执行已阻止。";
const CONTENT_RESULT_TTL_MS = 60_000;

type IdentityProfile = {
  principalType: "external_user" | "privileged_employee";
  targetAudience: "external" | "internal";
  channelClassification: "public" | "internal";
  allowedDataScopes: string[];
  userId: string;
  /** 企业/个人身份来源；fail-closed 时为 "fallback"（最低等级）。 */
  identitySource: "enterprise" | "personal" | "fallback";
  securityLevel: "public" | "internal" | "secret" | "top_secret";
  roleLevel: number;
};

type CachedContentResult = {
...
type ToolContentCheckInput = {
  ...
  profile: IdentityProfile;
  sequence: number;
  hook: "agent_tool_result_middleware" | "after_tool_call";
  metadata?: Record<string, unknown>;
};

type CachedContentCheck = {
  promise: Promise<SecurityResponse>;
  expiresAt: number;
};
...
type PluginRuntimeState = {
  ...
  identityBySession: Map<string, IdentityProfile>;
  contentResultByCall: Map<string, CachedContentResult>;
  contentCheckByCall: Map<string, CachedContentCheck>;
  blockedToolCalls: Map<string, number>;
  contextByToolCall: Map<string, ToolCallContext>;
  nextToolSequenceByTrace: Map<string, number>;
  /** token 消耗账本：modelRef -> 用量明细（供 :8000 /v1/local/usage 拉取）。 */
  usageLedger: Map<string, UsageRecord[]>;
};

const RUNTIME_STATE_KEY = "__clawguardAdapterRuntimeStateV1" as const;

function sharedRuntimeState(): PluginRuntimeState {
  ...
  const scope = globalThis as typeof globalThis & {
    [RUNTIME_STATE_KEY]?: PluginRuntimeState;
  };
  scope[RUNTIME_STATE_KEY] ??= {
    traceBySession: new Map<string, string>(),
    identityBySession: new Map<string, IdentityProfile>(),
    contentResultByCall: new Map<string, CachedContentResult>(),
    contentCheckByCall: new Map<string, CachedContentCheck>(),
    blockedToolCalls: new Map<string, number>(),
    contextByToolCall: new Map<string, ToolCallContext>(),
    nextToolSequenceByTrace: new Map<string, number>(),
...
function recordUsage(
  ...
  modelRef: string,
  usage: { input?: unknown; output?: unknown; cacheRead?: unknown; cacheWrite?: unknown; total?: unknown },
): void {
...
function mergeAttachments(
  primary: AttachmentDescriptor[],
  buffered: AttachmentDescriptor[],
  mediaRoot: string,
  enabled: boolean,
  ...
  const merged: AttachmentDescriptor[] = [];
  const push = (item: AttachmentDescriptor) => {
  ...
  const resolved: AttachmentDescriptor[] = [];
...
function extractMarkerLines(prompt: string): string[] {
  return prompt
    .split(/\r?\n/)
    .filter((line) => {
      return (
        /\[media attached:/i.test(line) ||
        /\[附件:/i.test(line) ||
        /-\s*图片:/i.test(line)
      );
    });
...
function defaultIdentityProfile(): IdentityProfile {
  return {
 ...
 * - 企业版：桌面登录后写 ~/.openclaw/clawguard/identity.json（user_id/security_level/specials），
...
function resolveIdentityProfile(input: {
  ...
  if (identity === null) {
    if (input.hadIdentityFileError) {
      input.logger.warn(
        "[clawguard-identity] identity file unreadable; fail-closed to public",
      );
    }
    return defaultIdentityProfile();
  }
  const isPrivileged =
    identity.securityLevel === "secret" ||
...
function rewrittenContent(
  ...
  const value = (response.data as Record<string, unknown>)[field];
  return typeof value === "string" ? value : undefined;
}

function cacheContentResponse(
  cache: Map<string, CachedContentResult>,
  key: string,
  response: SecurityResponse,
): void {
  cache.set(key, {
...
function removeExpiredResults(cache: Map<string, CachedContentResult>): void {
  const now = Date.now();
...
const plugin: OpenClawPluginDefinition = definePluginEntry({
  id: "clawguard-adapter",
  name: "Clawguard Adapter",
  description: "Connect OpenClaw security boundaries to the Clawguard API.",
  register(api) {
    ...
    const {
      ...
      blockedToolCalls,
      contextByToolCall,
      nextToolSequenceByTrace,
      usageLedger,
    } = sharedRuntimeState();
    const mediaRoot = resolveMediaRoot(config);
    const attachmentsBySession = new Map<string, AttachmentDescriptor[]>();

    const allocateToolSequence = (traceId: string): number => {
      const sequence = nextToolSequenceByTrace.get(traceId) ?? 0;
      nextToolSequenceByTrace.set(traceId, sequence + 1);
      return sequence;
    };

    const removeExpiredToolCallContexts = (): void => {
      const now = Date.now();
    ...
    const rememberToolCallContext = (
      toolCallId: string | undefined,
      context: Omit<ToolCallContext, "expiresAt">,
    ): void => {
    ...
    const forgetToolCallContext = (toolCallId: string | undefined): void => {
      if (toolCallId) {
        contextByToolCall.delete(toolCallId);
      }
    };

    const markToolCallBlocked = (toolCallId?: string): void => {
      if (toolCallId) {
        blockedToolCalls.set(
          toolCallId,
          Date.now() + CONTENT_RESULT_TTL_MS,
        );
      }
    };

    const isToolCallBlocked = (toolCallId?: string): boolean => {
      const now = Date.now();
    ...
    const consumeBlockedToolCall = (toolCallId?: string): boolean => {
    ...
    const removeExpiredContentChecks = (): void => {
      const now = Date.now();
    ...
    const checkToolContent = async (
      input: ToolContentCheckInput,
    ): Promise<SecurityResponse> => {
      const content = toolResultText(input.toolName, input.result);
      return client.checkContent(
        identityForRequest({
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        const sessionId =
          ctx.sessionKey ?? ctx.sessionId ?? "default_session";
        ...
        if (sessionId) {
          ...
          if (profile.userId) {
            ...
            if (bound && bound.userId && bound.userId !== profile.userId) {
              api.logger.warn(
                `[clawguard-session] owner mismatch session=${sessionId} ` +
        ...
        const attachments = mergeAttachments(
          parsePromptAttachments(originalPrompt),
          bufferedAttachments,
          mediaRoot,
          config.enableMediaCheck,
        );
        const checkText = stripGatewayEnvelope(event.prompt);
        try {
          const result = await client.checkInput(
            identityForRequest({ traceId, sessionId, profile }),
            {
              text: checkText,
              channel,
          ...
          api.logger.info(`clawguard-result ${JSON.stringify(result)}`);
          ...
          return {
            ...
            category: "clawguard_input",
          };
        } catch (error) {
          api.logger.error(
            `Clawguard input check failed: ${
          ...
          return {
            outcome: "block" as const,
            reason: "clawguard_service_unavailable",
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        const traceId =
          ...
          randomUUID();
        const profile =
          identityBySession.get(sessionId) ?? defaultIdentityProfile();
        ...
        try {
          const result = await client.checkTool(
            identityForRequest({ traceId, sessionId, profile }),
            {
              tool_name: toolName,
              ...
              identity_correlation_quality:
                profile.identitySource === "fallback"
                  ? "fallback"
                  : "identity_file",
            },
          );
          ...
          if (result.action === "human_review") {
            if (config.humanReviewAutoAllowTools.includes(event.toolName)) {
              api.logger.warn(
                `clawguard-human-review-auto-allowed ${JSON.stringify({
            ...
            return {
              requireApproval: {
                title: `Clawguard 请求确认工具：${event.toolName}`,
                description: result.reason || BLOCKED_TOOL_MESSAGE,
                severity: "warning" as const,
                timeoutBehavior: "deny" as const,
                pluginId: "clawguard-adapter",
          ...
          const rewrittenArguments =
            typeof data === "object" && data !== null
              ...
              : undefined;
          if (
            typeof rewrittenArguments === "object" &&
            rewrittenArguments !== null &&
            !Array.isArray(rewrittenArguments)
          ) {
            return { params: rewrittenArguments as Record<string, unknown> };
          }
          markToolCallBlocked(toolCallId);
          return {
            block: true,
            blockReason: "Clawguard 返回 rewrite，但未提供合法的新工具参数。",
          };
        } catch (error) {
          api.logger.error(
            `Clawguard tool pre-check failed: ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
          if (config.failMode === "open") {
    ...
    api.registerAgentToolResultMiddleware(
      async (event, ctx) => {
        ...
        const sequence =
          callContext?.sequence ?? allocateToolSequence(traceId);
        if (!callContext) {
        ...
        try {
          ...
          const replacement =
            result.action === "rewrite" && rewritten !== undefined
              ? rewritten
              : typeof result.risk_score === "number"
                ? `[Clawguard 已隔离此工具结果：内容安全检查未通过。score=${result.risk_score}]`
                : BLOCKED_CONTEXT_MESSAGE;
          return {
            result: replaceMessageText(
              event.result,
              replacement,
          ...
          api.logger.error(
            `Clawguard content middleware failed: ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
      ...
      async (event, ctx) => {
        ...
        try {
          try {
            ...
            api.logger.error(
              `Clawguard tool session recording failed: ${
                error instanceof Error ? error.message : String(error)
              }`,
            );
          }
          ...
          try {
            ...
            api.logger.error(
              `Clawguard content check failed: ${
                error instanceof Error ? error.message : String(error)
              }`,
            );
    ...
    api.on(
      ...
      (event, ctx) => {
        ...
        if (!result) {
          api.logger.warn(
            `Clawguard content result missing at persistence: tool=${toolName} key=${key}; applying failMode=${config.failMode}`,
          );
          if (config.failMode === "open") {
            return;
          }
        ...
        const replacement =
          result.action === "rewrite" && result.content !== undefined
            ? result.content
            : BLOCKED_CONTEXT_MESSAGE;
        return {
          message: replaceMessageText(
      ...
      (event) => {
        const role =
          typeof event.message === "object" && event.message !== null
            ? (event.message as { role?: unknown }).role
            : undefined;
        if (role === "user") {
          return;
        }
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        const profile =
          identityBySession.get(sessionId) ?? defaultIdentityProfile();
        try {
          const result = await client.checkOutput(
            identityForRequest({ traceId, sessionId, profile }),
            {
              text: event.content,
          ...
          if (
            ...
            return {
              ...
              metadata: {
                ...(event.metadata ?? {}),
                clawguardAction: result.action,
                clawguardReason: result.reason,
              },
            };
          }
          return {
          ...
          return {
            ...
            metadata: {
              ...(event.metadata ?? {}),
              clawguardAction: "service_unavailable",
            },
          };
        }
      },
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        try {
          ...
          api.logger.error(
            `Clawguard reply payload check failed: ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
          if (config.failMode === "open") {
    ...
    api.on("gateway_start", async () => {
      ...
      if (healthy) {
        api.logger.info("Clawguard API is healthy");
      } else {
        api.logger.warn("Clawguard API is unavailable; failMode applies");
      }
    });
    ...
    api.on("llm_output", (event) => {
      try {
        ...
        const ref =
          (typeof ev.resolvedRef === "string" && ev.resolvedRef) ||
