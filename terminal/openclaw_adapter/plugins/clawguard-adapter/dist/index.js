...
const BLOCKED_CONTEXT_MESSAGE = "[Clawguard 已隔离此工具结果：内容安全检查未通过。]";
const BLOCKED_TOOL_MESSAGE = "工具调用触发了安全策略，本次执行已阻止。";
const CONTENT_RESULT_TTL_MS = 60_000;
const RUNTIME_STATE_KEY = "__clawguardAdapterRuntimeStateV1";
...
function resolveIdentityProfile(input) {
    ...
    if (identity === null) {
        if (input.hadIdentityFileError) {
            input.logger.warn("[clawguard-identity] identity file unreadable; fail-closed to public");
        }
    ...
    register(api) {
        ...
        api.on("before_agent_run", async (event, ctx) => {
            ...
            if (sessionId) {
                ...
                if (profile.userId) {
                    ...
                    if (bound && bound.userId && bound.userId !== profile.userId) {
                        api.logger.warn(`[clawguard-session] owner mismatch session=${sessionId} ` +
            ...
            try {
                ...
                api.logger.error(`Clawguard input check failed: ${error instanceof Error ? error.message : String(error)}`);
            ...
            try {
                ...
                if (result.action === "human_review") {
                    if (config.humanReviewAutoAllowTools.includes(event.toolName)) {
                        api.logger.warn(`clawguard-human-review-auto-allowed ${JSON.stringify({
                ...
                api.logger.error(`Clawguard tool pre-check failed: ${error instanceof Error ? error.message : String(error)}`);
                if (config.failMode === "open") {
                    return;
                }
                markToolCallBlocked(toolCallId);
        ...
        api.registerAgentToolResultMiddleware(async (event, ctx) => {
            ...
            try {
                ...
                api.logger.error(`Clawguard content middleware failed: ${error instanceof Error ? error.message : String(error)}`);
        ...
        api.on("after_tool_call", async (event, ctx) => {
            ...
            try {
                try {
                    ...
                    api.logger.error(`Clawguard tool session recording failed: ${error instanceof Error ? error.message : String(error)}`);
                }
                if (!config.protectedTools.includes(event.toolName)) {
                    return;
                }
                ...
                try {
                    ...
                    api.logger.error(`Clawguard content check failed: ${error instanceof Error ? error.message : String(error)}`);
        ...
        api.on("tool_result_persist", (event, ctx) => {
            ...
            if (!result) {
                api.logger.warn(`Clawguard content result missing at persistence: tool=${toolName} key=${key}; applying failMode=${config.failMode}`);
                if (config.failMode === "open") {
                    return;
                }
                return {
        ...
        api.on("message_sending", async (event, ctx) => {
            ...
            try {
                ...
                api.logger.error(`Clawguard output check failed: ${error instanceof Error ? error.message : String(error)}`);
                if (config.failMode === "open") {
                    return;
                }
                return {
        ...
        api.on("reply_payload_sending", async (event, ctx) => {
            ...
            try {
                ...
                api.logger.error(`Clawguard reply payload check failed: ${error instanceof Error ? error.message : String(error)}`);
                if (config.failMode === "open") {
                    return;
                }
                return {
