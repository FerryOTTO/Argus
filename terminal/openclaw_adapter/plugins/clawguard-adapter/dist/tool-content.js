const EXTERNAL_WRAPPER_RE = /<<<(?:END_)?EXTERNAL_UNTRUSTED_CONTENT\b[^>]*>>>/gi;
const EXTERNAL_BLOCK_RE = /<<<EXTERNAL_UNTRUSTED_CONTENT\b[^>]*>>>\s*(?:Source:\s*Web (?:Search|Fetch)\s*)?(?:---\s*)?([\s\S]*?)\s*<<<END_EXTERNAL_UNTRUSTED_CONTENT\b[^>]*>>>/gi;
...
function decodeTransportNewlines(value) {
...
function unwrapExternalContent(value) {
...
function isTransportError(value, allowUntypedError = false) {
    const parsed = parseStructuredValue(value);
    if (parsed !== value) {
...
function resultText(value) {
    ...
    const fields = ["title", "description", "excerpt", "content"];
    const lines = fields
...
function genericToolResultText(value) {
    if (typeof value === "string") {
        return value;
    }
    const record = asRecord(value);
...
export function toolResultText(toolName, value) {
