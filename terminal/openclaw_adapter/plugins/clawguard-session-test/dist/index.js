/**
 * Clawguard 全链路安全与访问控制拦截插件（OpenClaw 适配层）。
 * 
 * 功能：
 *   1. 输入阶段 (message_received)：绑定原始 Prompt 到 Tool Guard 意图会话，并上报审计事件；
 *   2. 工具前置 (before_tool_call)：同步请求 Clawguard /v1/tool/pre_check 进行 RBAC/MAC 鉴权与审计风险联动：
...
const CLAWGUARD_URL = "http://127.0.0.1:8000";

// 从 hook 的 (event, ctx) 提取 Clawguard 审计/上下文通用字段
function extractAuditFields(stage, event, ctx) {
  ...
  const userId = ctx.userId ?? ctx.customHeaders?.["x-clawguard-user-id"] ?? ctx.headers?.["x-clawguard-user-id"] ?? "user_01";
...
async function postAudit(audit) {
  try {
    await fetch(`${CLAWGUARD_URL}/v1/audit/event`, {
      method: "POST",
...
async function bindPrompt(sessionId, prompt, traceId) {
  try {
    await fetch(`${CLAWGUARD_URL}/v1/tool/session/prompt`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export default definePluginEntry({
  id: "clawguard-session-test",
  name: "Clawguard Access Control & Audit Plugin",
  ...
  register(api) {
    ...
    // ② 工具调用前：同步调用 Clawguard /v1/tool/pre_check 进行访问控制与防线升级判定
    api.on("before_tool_call", async (event, ctx) => {
      ...
      try {
        const resp = await fetch(`${CLAWGUARD_URL}/v1/tool/pre_check`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            context: {
        ...
        if (resp.ok) {
          ...
          if (action === "block") {
            ...
            return {
              block: true,
              blockReason: `[Clawguard 安全拦截] 访问控制拒绝执行工具 "${toolName}": ${reason}`,
            };
          }
          ...
          if (action === "human_review") {
            ...
            return {
              block: true,
              blockReason: `[Clawguard 二次审批] 触发防线升级二次审批: ${reason}`,
            };
          }
        ...
        console.error("[Clawguard Plugin] pre_check error:", err.message);
