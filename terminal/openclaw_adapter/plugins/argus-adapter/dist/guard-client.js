...
export class GuardClient {
    ...
    async reportUsage(records) {
        ...
        if (!response.ok) {
            throw new GuardServiceError(`Argus returned HTTP ${response.status} for /v1/local/usage/ingest`);
        }
    }
    ...
    async check(stage, path, identity, payload) {
        ...
        if (!response.ok) {
            throw new GuardServiceError(`Argus returned HTTP ${response.status} for ${path}`);
        }
        const value = (await response.json());
        if (!this.isSecurityResponse(value, stage)) {
            throw new GuardServiceError(`Argus returned an invalid response for ${path}`);
        }
        return value;
    }
    ...
    async request(path, body, method) {
        ...
        try {
            return await fetch(`${this.config.argusUrl}${path}`, {
                method,
                body,
