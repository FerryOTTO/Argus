...
export function mediaUriName(uri: string): string {
  return basename(uri);
}

/** Parse webchat / QQ media markers out of the user prompt. */
export function parsePromptAttachments(prompt: string): AttachmentDescriptor[] {
  const found: AttachmentDescriptor[] = [];
  ...
  const push = (value: string) => {
    ...
    const descriptor: AttachmentDescriptor = { name };
...
export function resolveMediaRoot(cfg: { mediaRoot?: string }): string {
...
export function resolveMediaUri(uri: string, mediaRoot: string): string | null {
 ...
 */
export function stripGatewayEnvelope(prompt: string): string {
  return prompt
    .replace(
      /Conversation info\s*\(?untrusted metadata\)?:[\s\S]*?```[\s\S]*?```/gi,
      "",
    )
...
export function attachmentsFromMetadata(meta: unknown): AttachmentDescriptor[] {
  if (typeof meta !== "object" || meta === null) {
    return [];
  }
  const record = meta as Record<string, unknown>;
  const collected: Array<{ value: string; type?: string }> = [];
  const collect = (valueKey: string, typeKey: string) => {
    const raw = record[valueKey];
    const types = record[typeKey];
    const pickType = (index: number): string | undefined => {
