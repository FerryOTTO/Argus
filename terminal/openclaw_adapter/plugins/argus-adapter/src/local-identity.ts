...
export const PERSONAL_IDENTITY: LocalIdentity = {
...
const LEVEL_ALIASES: Record<string, SecurityLevel> = {
  "1": "public",
  a: "public",
  public: "public",
  "2": "internal",
...
export const SECURITY_LEVEL_NUMBERS: Record<SecurityLevel, number> = {
  public: 1,
  internal: 2,
  secret: 3,
  top_secret: 4,
...
export function normalizeSpecials(raw: unknown): string[] {
...
function parseIdentityFile(content: string): LocalIdentity | null {
  let raw: unknown;
  try {
...
export function identityFileCandidates(explicitPath?: string): string[] {
  if (explicitPath && explicitPath.trim()) {
    return [explicitPath];
  }
  const fromEnv = process.env["ARGUS_IDENTITY_FILE"];
  if (fromEnv && fromEnv.trim()) {
    return [fromEnv];
  }
  // 约定位置：网关数据目录，与 mediaRoot（~/.openclaw/media）同一父目录。
  return [path.join(os.homedir(), ".openclaw", "argus", IDENTITY_FILE_NAME)];
}
 ...
 */
export function readLocalIdentity(explicitPath?: string): LocalIdentity | null {
