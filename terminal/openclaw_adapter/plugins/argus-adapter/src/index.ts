...
const RUNTIME_STATE_KEY = "__argusAdapterRuntimeStateV1" as const;
...
function resolveIdentityProfile(input: {
  ...
  if (identity === null) {
    if (input.hadIdentityFileError) {
      input.logger.warn(
        "[argus-identity] identity file unreadable; fail-closed to public",
      );
    }
  ...
  register(api) {
        ...
        if (sessionId) {
          ...
          if (profile.userId) {
            ...
            if (bound && bound.userId && bound.userId !== profile.userId) {
              api.logger.warn(
                `[argus-session] owner mismatch session=${sessionId} ` +
        ...
        try {
          ...
          if (result.action === "human_review") {
            if (config.humanReviewAutoAllowTools.includes(event.toolName)) {
              api.logger.warn(
                `argus-human-review-auto-allowed ${JSON.stringify({
    ...
    api.registerAgentToolResultMiddleware(
      async (event, ctx) => {
        ...
        try {
          ...
          api.logger.error(
            `Argus content middleware failed: ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
      ...
      async (event, ctx) => {
        ...
        try {
          try {
            ...
            api.logger.error(
              `Argus tool session recording failed: ${
                error instanceof Error ? error.message : String(error)
              }`,
            );
          }
    ...
    api.on(
      ...
      (event, ctx) => {
        ...
        if (!result) {
          api.logger.warn(
            `Argus content result missing at persistence: tool=${toolName} key=${key}; applying failMode=${config.failMode}`,
          );
          if (config.failMode === "open") {
            return;
          }
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        try {
          ...
          api.logger.error(
            `Argus reply payload check failed: ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
          if (config.failMode === "open") {
