# OpenClaw Argus Adapter

该插件负责在 OpenClaw 与统一 Argus FastAPI 之间转换请求和执行动作，不再
直接调用 IO Guard Sidecar。风险判断仍以 Argus 为准；插件仅保留一组与后端占位符
一致的确定性脱敏规则，用于无法异步请求 HTTP 的会话持久化 Hook。

## 统一接口

| OpenClaw Hook | Argus 接口 | 行为 |
|---|---|---|
...
`prompt` 的修改。插件收到 Argus `rewrite` 时把 `event.prompt` 改成 `data.text` 并
返回标准 `{ outcome: "pass" }`；兼容补丁会在模型调用前消费这个净化后的输入，同时
保留原始用户消息以便审计。

兼容补丁和应用说明位于
`openclaw_adapter/patch_source/argus-adapter/README.md`。未应用补丁时，插件会
...
给 Argus 检测。边界标记不会再作为自然文本触发误报，但标记内部的真实提示
注入指令仍会被保留并检测。
...
尚未实现 Bearer Token，可将 `ARGUS_API_TOKEN` 留空。
