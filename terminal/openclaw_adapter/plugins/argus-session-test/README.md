# Argus Session Test（审计数据源插件）

OpenClaw 侧全流程审计数据源：把输入 / 工具调用 / 输出事件上报 Argus `/v1/audit/event`。
...
```powershell
cd Argus
openclaw plugins install .\openclaw_adapter\plugins\argus-session-test --force
openclaw gateway stop
schtasks /End /TN "OpenClaw Gateway"   # 如安装过计划任务
openclaw gateway run
```

启动日志确认插件列表包含 `argus-session-test`。

## 4. 验证

触发一轮对话（如 web_fetch + 总结），Argus 窗口应出现多条 `POST /v1/audit/event 200 OK`；
落盘文件 `runtime/audit/<日期>.jsonl` 中出现同一 `trace_id` 的 input/tool/output 事件链。

## 5. 说明

- 命名仍是 `argus-session-test`（观测期名称）；正式化时可改名为 `argus-audit`
- 上报目标地址 `ARGUS_URL` 在 `dist/index.js` 顶部，可改
