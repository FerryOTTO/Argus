...
> ⚠️ **已废弃（2026-08-20）**：content 检查改由 `argus-adapter` 插件（`agentToolResultMiddleware`）
> 转发到服务端 Retrieval Guard adapter，**不再需要 bundle 补丁**。OpenClaw 端当前无需打任何补丁
> （仅需安装插件 + 服务端运行）。本目录保留作为回退方案（bundle 还原 + 打补丁 = 旧方案复活）。
>
> **现行方案（新思路）**：
> 1. OpenClaw 端装 `argus-adapter` 插件（compat 已适配 6.11），middleware 转发 `/v1/content/check`
...
> 4. 插件与 6.11 适配说明见 `openclaw_adapter/patch_source/argus-adapter/README.md`
...
2. POST `http://127.0.0.1:8000/v1/content/check`（Argus 统一检索安全检查）
...
patch -p1 < "e:/OpenClaw/Argus/openclaw_adapter/patches/openclaw-2026.6.11-retrieval-guard.patch"
...
```bash
cd "e:/OpenClaw/npm-global/node_modules/openclaw"
patch -p1 -R < "e:/OpenClaw/Argus/openclaw_adapter/patches/openclaw-2026.6.11-retrieval-guard.patch"
```
...
| `before_agent_run` | ⚠️ 能 block；prompt 改写需补丁消费（6.11 等价补丁已移植，见 `patch_source/argus-adapter/`） |
...
- Argus 服务运行在 `127.0.0.1:8000`（`cd Argus && python -m uvicorn argus.api.main:app --port 8000`）
- Argus 不可达时补丁 fail-open，OpenClaw 正常运作但不检测
