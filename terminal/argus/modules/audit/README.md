...
Audit 模块接收并保存 Argus 全链路产生的统一 `AuditEvent`，按 `trace_id`
...
```powershell
$env:ARGUS_AUDIT_DIR = "runtime/audit"
$env:ARGUS_AUDIT_PATH = "runtime/audit/custom.jsonl"
$env:ARGUS_AUDIT_RISK_THRESHOLD = "0.5"
```

`ARGUS_AUDIT_PATH` 优先于 `ARGUS_AUDIT_DIR`。阈值必须位于 0～1；当
...
```python
from argus.modules.audit.original import AuditQuery
...
```powershell
.\.venv\Scripts\python.exe -m uvicorn argus.api.main:app --host 127.0.0.1 --port 8000
```
...
python -m pip install -r argus/modules/audit/requirements.txt
...
python -m pytest -q
python -m compileall -q argus tests
python -m pip check
git diff --check
```
