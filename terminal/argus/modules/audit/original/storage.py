"""Validated, append-only JSONL storage for Argus AuditEvent records."""
...
from argus.common.models import AuditEvent
...
def resolve_audit_path(path: str | Path | None = None) -> Path:
    ...
    if path is not None:
        configured = path
    elif os.getenv("ARGUS_AUDIT_PATH"):
        configured = os.environ["ARGUS_AUDIT_PATH"]
    else:
        configured = os.getenv("ARGUS_AUDIT_DIR", str(DEFAULT_AUDIT_DIR))
...
def get_risk_threshold(value: float | str | None = None) -> float:
    raw: float | str = (
        ...
        if value is not None
            ...
            "ARGUS_AUDIT_RISK_THRESHOLD", str(DEFAULT_RISK_THRESHOLD)
        )
    )
    try:
        threshold = float(raw)
