...
PATHS = {
  ...
  "policy": _p("argus", "modules", "io_guard", "original", "configs", "default_policy.json"),
  "whitelist": _p("argus", "modules", "retrieval_guard", "original", "a_url", "whitelist.yaml"),
  "b_injection": _p("argus", "modules", "retrieval_guard", "original", "b_injection", "config.yaml"),
  "c_prompt": _p("argus", "modules", "retrieval_guard", "original", "c_prompt", "config.yaml"),
  "users": _p("argus", "modules", "access_control", "original", "rules", "users.txt"),
  "resources": _p("argus", "modules", "access_control", "original", "rules", "resources.txt"),
...
def build_snapshot():
  ...
  return {
    ...
    "integration": integration if integration else {"argus_url": "http://127.0.0.1:8000", "api_token_env": "ARGUS_API_TOKEN", "timeout_ms": 30000, "fail_mode": "closed", "enable_media_check": True, "media_root": "", "protected_tools": ["web_fetch", "web_search"]},
...
def enterprise_status():
  ...
  try:
    ...
    _afs = _glob.glob(os.path.join(ROOT, "argus", "audit", "*.jsonl")) + _glob.glob(os.path.join(ROOT, "runtime", "audit", "*.jsonl")) + _glob.glob(os.path.join(ROOT, "logs", "audit*.jsonl"))
    _cnt = 0
    for _fp in _afs[:6]:
      try:
