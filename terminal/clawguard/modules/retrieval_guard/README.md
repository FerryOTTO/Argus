...
对接 Adapter：[clawguard/adapters/retrieval_guard_adapter.py](../../adapters/retrieval_guard_adapter.py)

## 3. 依赖与安装

```powershell
python -m pip install -r clawguard\modules\retrieval_guard\requirements.txt
```
...
**环境建议**：使用 conda 环境 `face_test`（Python 3.11 + CUDA 已就绪）。Clawguard 统一 FastAPI 必须跑在装有 torch 的环境中（Adapter 直接 import PIGuard）。
...
| 目标目录 | `clawguard/modules/retrieval_guard/models/PIGuard`（**不提交 Git**，`.gitignore` 已排除） |
...
retrieval_guard:
  ...
  model_path: clawguard/modules/retrieval_guard/models/PIGuard   # 不填用默认约定目录
  guards:
    A: true   # URL 白名单
    B: true   # PIGuard 注入检测
    C: true   # 提示词包装
...
```powershell
cd ClawguardV2.1
E:\AnacondaEnvs\face_test\python.exe -m uvicorn clawguard.api.main:app --host 127.0.0.1 --port 8000
```
...
- **接入方式**：`clawguard-adapter` 插件的 `agentToolResultMiddleware` 把 `web_search`/`web_fetch` 结果转发到 `/v1/content/check`，服务端由本模块 Adapter（PIGuard）处理
...
- 插件与 6.11 适配说明：`openclaw_adapter/patch_source/clawguard-adapter/README.md`
