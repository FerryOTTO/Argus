...
直接编辑规则文本文件（Clawguard 规则引擎后台具备基于 `mtime` 的毫秒级文件变动自动热重载能力）：
* 文件路径：`clawguard/modules/access_control/original/rules/users.txt`
...
2. **规则修改专属 Tab**：
   ...
   * Clawguard 引擎基于 `mtime` 文件监听在毫秒级内自动热重构内存规则树，业务零中断热生效。
