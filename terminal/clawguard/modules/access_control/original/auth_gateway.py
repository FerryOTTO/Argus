...
def _resolve_default_user_level():
    try:
        raw=os.getenv("CLAWGUARD_DEFAULT_USER_LEVEL","").strip().lower()
        if raw and raw in _LEVEL_ALIASES:
            return _LEVEL_ALIASES[raw]
    except Exception:
        pass
    try:
        cfgp=BASE_DIR.parent.parent.parent.parent/"configs"/"access_local.json"
        if cfgp.exists():
            d=json.loads(cfgp.read_text(encoding="utf-8"))
            v=str(d.get("default_user_level","")).strip().lower()
            if v in _LEVEL_ALIASES:
                return _LEVEL_ALIASES[v]
    except Exception:
        pass
    return 1

DEFAULT_USER_LEVEL = _resolve_default_user_level()


# 本地单用户绑定（users.txt 下线后，终端只认这一条 + resources.txt）
# 企业版由服务端下发 access_user 写入；个人版由“默认用户等级”兜底，bound_user 为空即走默认。

def _access_local_path():
    try:
        return BASE_DIR.parent.parent.parent.parent / "configs" / "access_local.json"
    except Exception:
        return None


def _load_bound_user() -> dict:
    try:
        fp = _access_local_path()
        if fp is None or not fp.exists():
            return {}
        d = json.loads(fp.read_text(encoding="utf-8"))
        b = d.get("bound_user") if isinstance(d, dict) else None
        if not isinstance(b, dict):
            return {}
        name = str(b.get("username") or "").strip()
        if not name:
            return {}
        specials = b.get("specials") or []
        if isinstance(specials, str):
            specials = [s for s in (x.strip() for x in specials.split(",")) if s]
        else:
            specials = [str(s).strip() for s in specials if str(s).strip()]
        return {"username": name, "level": _normalize_level(b.get("level") or "internal"), "specials": specials}
    except Exception:
        return {}


def _save_bound_user(username: str, level_name: str, specials=None) -> bool:
    try:
        fp = _access_local_path()
        if fp is None:
            return False
        ...
        if not isinstance(d, dict):
            d = {}
        sp = specials if isinstance(specials, list) else []
        new = {"username": username, "level": level_name, "specials": sp}
        if d.get("bound_user") == new:
            return False
        d["bound_user"] = new
        try:
            if fp.exists():
                fp.with_suffix(".json.bak").write_text(fp.read_text(encoding="utf-8"), encoding="utf-8")
        except Exception:
            pass
...
class _TextBackend(_RuleBackend):
    ...
    def __init__(self, users_file: Path, resources_file: Path):
        ...
        self._last_users_mtime = 0.0
        self._last_resources_mtime = 0.0
    ...
    def load_users(self) -> dict[str, dict]:
        ...
        if not self.users_file.exists():
            return users
        try:
            self._last_users_mtime = self.users_file.stat().st_mtime
        except Exception:
            pass
    ...
    def load_resources(self) -> list[tuple]:
        ...
        if not self.resources_file.exists():
            return resources
        try:
            self._last_resources_mtime = self.resources_file.stat().st_mtime
        except Exception:
            pass
...
class RuleStore:
    ...
    users: dict[str, dict] = {}
    resources: list[tuple] = []
    mac_labels: dict[str, SecurityLabel] = {}
    resource_labels: dict[str, SecurityLabel] = {}
    ...
    def _check_auto_reload(self) -> None:
        if isinstance(self._backend, _TextBackend):
            try:
                u_mtime = self._backend.users_file.stat().st_mtime if self._backend.users_file.exists() else 0.0
                r_mtime = self._backend.resources_file.stat().st_mtime if self._backend.resources_file.exists() else 0.0
                if u_mtime != self._backend._last_users_mtime or r_mtime != self._backend._last_resources_mtime:
                    self.reload()
            except Exception:
                pass
    ...
    def reload(self) -> None:
        with self._lock:
            ...
            self.bound = _load_bound_user()
            try:
                global DEFAULT_USER_LEVEL
                DEFAULT_USER_LEVEL=_resolve_default_user_level()
            except Exception:
                pass

    def _lookup_db_user(self, user_id: str) -> dict | None:
        """从 OpenGuard 数据库动态回查用户安全等级与特例（支持 id、username、agent_id）"""
        try:
            db_candidates = [
                Path(__file__).resolve().parent.parent.parent.parent.parent / "openguard" / "original" / "data" / "auth.db",
                Path("openguard/original/data/auth.db").resolve(),
            ]
            ...
            cur = conn.cursor()
            cur.execute("SELECT security_level, role FROM users WHERE id = ? OR username = ?", [user_id, user_id])
            row = cur.fetchone()
            if not row:
                cur.execute("SELECT u.security_level, u.role FROM users u JOIN user_agents ua ON u.id = ua.user_id WHERE ua.agent_id = ?", [user_id])
                row = cur.fetchone()
            conn.close()
            if row:
                sec_lvl, role = row[0], row[1]
                lvl_num = _normalize_level(sec_lvl)
                specials = ["*"] if lvl_num == 4 or role == "admin" else []
                info = {"level": lvl_num, "specials": specials}
    ...
    def get_user_level(self, user_id: str) -> int | None:
        ...
        with self._lock:
            b = self.bound or {}
            if user_id and b.get("username") and user_id == b["username"]:
                return b["level"]
            u = self.users.get(user_id)
            if not u:
                u = self._lookup_db_user(user_id)
    ...
    def set_user_level(self, user_id: str, level: int) -> None:
        with self._lock:
            ...
            if user_id and b.get("username") and user_id == b["username"]:
                try:
                    lname = {1: "public", 2: "internal", 3: "secret", 4: "top_secret"}.get(int(level), "internal")
                except Exception:
                    lname = "internal"
                if _save_bound_user(user_id, lname, b.get("specials")):
                    self.bound = _load_bound_user()
                return
    ...
    def add_user_rule(self, user_id: str, level: str, specials: list[str] = None) -> dict:
        with self._lock:
            ...
            if specials is None:
                final_specials = existing.get("specials", [])
            else:
                final_specials = [s.strip() for s in specials if s.strip()]
            self.users[user_id] = {"level": _normalize_level(level), "specials": final_specials}
            self._backend.save_users(self.users)
...
def set_bound_user(username: str, level_name: str, specials=None) -> bool:
    if _save_bound_user(username, level_name, specials or []):
        try:
            _store.bound = _load_bound_user()
        except Exception:
            pass
        return True
    return False
...
def _check_special(specials: list[str], path: str, tool: str, database: str) -> str | None:
    candidates = [p for p in (path, tool, database) if p]
    # 显式否定优先：先扫描所有否定规则 (!pattern)，命中立即短路拒绝
