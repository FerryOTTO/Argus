...
def resolve_quarantine_path(path: str | Path | None = None) -> Path:
    ...
    if path is not None:
        configured = path
    elif os.getenv('CLAWGUARD_QUARANTINE_PATH'):
        configured = os.environ['CLAWGUARD_QUARANTINE_PATH']
    else:
        configured = os.getenv('CLAWGUARD_QUARANTINE_DIR', str(DEFAULT_QUARANTINE_DIR))
