...
# 近永久隔离（仅在 5min 滑动窗口之上叠加，不替代）
DEFAULT_QUARANTINE_ENABLED = os.getenv("CLAWGUARD_QUARANTINE_ENABLED", "1") != "0"
DEFAULT_QUARANTINE_BLOCK_COUNT = int(os.getenv("CLAWGUARD_QUARANTINE_BLOCK_COUNT", "8"))
DEFAULT_QUARANTINE_RISK_THRESHOLD = float(os.getenv("CLAWGUARD_QUARANTINE_RISK_THRESHOLD", "0.85"))
DEFAULT_QUARANTINE_LONG_WINDOW_SECONDS = int(os.getenv("CLAWGUARD_QUARANTINE_LONG_WINDOW_SECONDS", "3600"))
...
# ========== 近永久隔离：5min 窗口保留，叠加永久标记 ==========

class QuarantinePolicy:
    """决定何时将用户打上近永久标记（需管理员解除）。"""
    ...
    def __init__(
        ...
        *,
        ...
        risk_threshold: float | None = None,
        long_window_block_count: int | None = None,
    ) -> None:
        self.enabled = enabled if enabled is not None else DEFAULT_QUARANTINE_ENABLED
        self.block_count = block_count if block_count is not None else DEFAULT_QUARANTINE_BLOCK_COUNT
        self.risk_threshold = risk_threshold if risk_threshold is not None else DEFAULT_QUARANTINE_RISK_THRESHOLD
        self.long_window_block_count = long_window_block_count if long_window_block_count is not None else int(os.getenv("CLAWGUARD_QUARANTINE_LONG_WINDOW_COUNT", "15"))

    def should_quarantine(self, assessment: dict[str, Any]) -> tuple[bool, str]:
        if not self.enabled:
            return False, ""
        bc = int(assessment.get("block_count", 0))
        rs = float(assessment.get("risk_score", 0.0))
        probe = bool(assessment.get("probe_likely", False))
        if bc >= self.block_count:
            return True, f"quarantine: 5min window block_count={bc} >= {self.block_count}"
        if probe and rs >= self.risk_threshold:
            return True, f"quarantine: risk {rs:.2f} >= {self.risk_threshold:.2f} probe block={bc}"
        return False, ""

    def long_window_assessment(self, user_id: str, session_id: str = "", *, store=None, window_seconds: int | None = None) -> dict[str, Any]:
        from clawguard.modules.audit.original.storage import parse_timestamp
        ws = window_seconds if window_seconds is not None else DEFAULT_QUARANTINE_LONG_WINDOW_SECONDS
        cutoff = datetime.now(timezone.utc) - timedelta(seconds=ws)
        audit_store = store or AuditStore()
        if callable(audit_store) and not isinstance(audit_store, AuditStore):
            audit_store = audit_store()
        recent = []
        for ev in audit_store.load():
            ...
            if ts >= cutoff:
                recent.append(ev)
        bc = sum(1 for e in recent if e.get("action") == "block")
        if bc >= self.long_window_block_count:
            return {"trigger": True, "reason": f"quarantine: 1h window block={bc} >= {self.long_window_block_count}", "block_count": bc, "window_seconds": ws}
        return {"trigger": False, "block_count": bc, "window_seconds": ws}
