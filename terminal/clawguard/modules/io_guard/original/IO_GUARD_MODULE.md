...
- **模块路径**：`clawguard/modules/io_guard/original/`
...
- **启动方式**：作为 Clawguard FastAPI 的一个模块适配器被调用；也可独立以 HTTP 服务运行（`service.py` 提供 `ThreadingHTTPServer`，默认端口 8765）。生产环境经 `clawguard/api/main.py` 暴露 `/v1/input/check`、`/v1/content/check`、`/v1/output/check`。
...
│   ├── events.py / clawguard_contracts.py / runtime.py
│   ├── adapters/
│   │   ├── base.py
│   │   └── clawguard_adapter.py     # 内层适配器：Clawguard 契约 ↔ GuardService
│   ├── detectors/
│   │   ├── base.py
...
### 10.1 适配器编排（`clawguard/adapters/io_guard_adapter.py`）
...
内层 `io_guard/adapters/clawguard_adapter.py` 与 `service.py` 保持不动，仅做契约转换（`payload.text|content` → `GuardRequest`）。

### 10.2 格式与提取器（`clawguard/modules/media_input/`）
...
- `test_clawguard_adapter.py`、`test_service.py`：适配器契约与 HTTP 门面。
...
5. **Editable 安装路径**：切换工作副本后应在当前仓库根目录重新执行 `python -m pip install -r clawguard\modules\io_guard\requirements.txt`，避免 Python 环境继续加载其他 checkout 中的旧 `io_guard` 包。
