...
def _audit_count():
    try:
        files = (_glob.glob(os.path.join(ROOT, "clawguard", "audit", "*.jsonl"))
...
class RegisterBody(BaseModel):
    ...
    clawguard_version: Optional[str] = "2.1"
...
def remote_register(body: RegisterBody):
    ...
    payload = {"registration_code": code, "hostname": body.hostname or "", "os_info": body.os_info or "", "agent_type": body.agent_type or "", "agent_version": body.agent_version or "", "clawguard_version": body.clawguard_version or "2.1"}
    try:
...
def remote_sync_now():
    ...
    try:
        from clawguard.api.remote_sync import run_once
        ok = run_once()
