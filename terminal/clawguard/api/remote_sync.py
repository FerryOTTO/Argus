...
契约见 LLMGate/REMOTE.md 4.2/4.5/4.7，上报周期默认 60 秒（环境变量 CLAWGUARD_SYNC_INTERVAL）。
...
log = logging.getLogger("clawguard.remote_sync")
...
USERS_PATH = os.path.join(ROOT, "clawguard", "modules", "access_control", "original", "rules", "users.txt")
RESOURCES_PATH = os.path.join(ROOT, "clawguard", "modules", "access_control", "original", "rules", "resources.txt")
MODULES_PATH = os.path.join(ROOT, "configs", "modules.yaml")
POLICY_PATH = os.path.join(ROOT, "clawguard", "modules", "io_guard", "original", "configs", "default_policy.json")
...
WHITELIST_PATH = os.path.join(ROOT, "clawguard", "modules", "retrieval_guard", "original", "a_url", "whitelist.yaml")
B_INJECTION_PATH = os.path.join(ROOT, "clawguard", "modules", "retrieval_guard", "original", "b_injection", "config.yaml")
C_PROMPT_PATH = os.path.join(ROOT, "clawguard", "modules", "retrieval_guard", "original", "c_prompt", "config.yaml")
...
def _interval():
    try:
        return max(15, int(os.environ.get("CLAWGUARD_SYNC_INTERVAL", "60") or 60))
    except ValueError:
        return 60
...
def do_config(base, token, cursors):
    try:
        ...
        try:
            ...
            def _reload():
                try:
                    from clawguard.core import registry as _regmod
                ...
                if bool(isinstance(obj.get("io_guard_policy"), dict)) or bool(isinstance(obj.get("retrieval"), dict)):
                    try:
                        from clawguard.core import registry as _regmod2
                        ...
                        if reg2 is not None:
                            ...
                            try:
                                ...
                                if _rt is not None:
                                    try:
                                        from clawguard.modules.retrieval_guard.original.a_url import URLWhitelist as _UWL
                                        _rt._url_guard = _UWL()
                                    except Exception:
                                        pass
                                    try:
                                        from clawguard.modules.retrieval_guard.original.c_prompt import PromptWrapper as _PW
                                        _rt._wrapper = _PW()
                                    except Exception:
                                        pass
                            except Exception:
                ...
                try:
                    from clawguard.api import main as _main
...
def do_heartbeat(base, token):
    ...
    ok, code, data = _post(base, token, "/telemetry/v1/heartbeat", {
        ...
        "agent_version": "", "clawguard_version": "2.1",
    })
    if ok:
