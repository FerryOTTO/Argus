...
from clawguard.api.quarantine_routes import router as quarantine_router
from clawguard.api.tool_guard_routes import router as tool_guard_router
from clawguard.api.local_config_routes import router as local_config_router
from clawguard.api.openclaw_providers_routes import router as openclaw_providers_router
from clawguard.api.remote_routes import router as remote_router
from clawguard.api.skills_routes import router as skills_router
from clawguard.api.usage_routes import router as usage_router
from clawguard.common.models import (
    Action,
    AuditEvent,
    ModuleResult,
    RequestContext,
...
from clawguard.core.registry import AdapterRegistry, registry
from clawguard.modules.audit.original import AuditStore
from clawguard.modules.audit.integration import (
...
async def _lifespan(app: FastAPI):
    try:
        from clawguard.api.remote_sync import start_remote_sync
        start_remote_sync(app)
    except Exception:
        pass
    yield
    try:
        from clawguard.api.remote_sync import stop_remote_sync
...
app = FastAPI(
    ...
    title="Clawguard API",
    version="0.1.0",
    description="Clawguard 各安全模块的统一测试入口。",
)
...
def get_desktop_runtime() -> dict[str, Any]:
    ...
    无账本数据时返回全零刻度，不编数。
    """
    from clawguard.api import usage_routes as _usage

    data = _usage.read_usage_buckets()
    return {
        "source": "clawguard-usage-ledger",
