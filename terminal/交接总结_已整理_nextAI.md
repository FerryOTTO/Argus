# Clawguard V2.1 工作交接总结（已整理 NextAI 版）

> 仓库：E:/tiaozhanbei/MAC/ClawguardV2.1（git 真仓库） 远端 https://github.com/WT-ever/ClawguardV2.1.git
...
- Clawguard clawguard/ FastAPI+uvicorn

## 二、启动顺序
1. openclaw gateway run (daemon, 需 stop 否则 18789 占用)
2. py -m uvicorn clawguard.api.main:app --host 127.0.0.1 --port 8000
...
- 规则文件 clawguard/modules/access_control/original/rules/users.txt
- 新注册默认 internal，需在 users.txt 配等级并重启 Clawguard
- 验证：query_weather 全放行, read_file>=internal, write_file>=secret, execute_bash>=top_secret

## 五、插件
- 源码 openclaw_adapter/plugins/clawguard-adapter (compat >=2026.6.11)
- 部署 C:/Users/admin/.openclaw/extensions/clawguard-adapter dist+manifest+package.json
- 配置 openclaw.json allow clawguard-adapter + hooks.allowConversationAccess=true
...
- penalty 参数向后兼容，fail-open，CLAWGUARD_AC_RISK_LINK=0 关闭
...
- Clawguard：统一安全服务，clawguard/ 目录，FastAPI + uvicorn。
启动顺序：先 OpenClaw，再 Clawguard 和 OpenGuard，最后 Bridge（Bridge 依赖 OpenClaw ready 才能握手）。
...
2. Clawguard（后台）： cd /e/tiaozhanbei/MAC/ClawguardV2.1 && py -m uvicorn clawguard.api.main:app --host 127.0.0.1 --port 8000 注意：python 命令要用 py（Windows 上 python 指向 Python 2.7，py 才是 Python 3.13）。
3. OpenGuard（后台）： cd /e/tiaozhanbei/MAC/ClawguardV2.1/openguard/original && py -m uvicorn main:app --host 127.0.0.1 --port 3000
4. Bridge（后台）： cd /e/tiaozhanbei/MAC/ClawguardV2.1/openguard/original && node bridge.js
...
- test_public → public， id=usr_sZdr4JGexJ2hDQ5p 访问控制规则文件：E:\tiaozhanbei\MAC\ClawguardV2.1\clawguard\modules\access_control\original\rules\users.txt 新注册的 OpenGuard 用户 security_level 都是 internal（auth.py register_user 里硬编码），要分等级需在 users.txt 里给对应 usr_xxx 配等级，然后重启 Clawguard（规则在启动时加载，无 reload 接口）。 访问控制等级阶梯已验证正确：query_weather 四级全放行；read_file 需 internal；write_file 需 secret；execute_bash 需 top_secret。
插件源码：E:\tiaozhanbei\MAC\ClawguardV2.1\openclaw_adapter\plugins\clawguard-adapter 插件本就是为 2026.6.11 构建（package.json 里 compat.pluginApi >=2026.6.11），文档《真实链路接入方案.md》说按 2026.7.1-2 构建是过时的。 部署位置：C:\Users\admin.openclaw\extensions\clawguard-adapter\（注意是 extensions 不是 plugins） 部署内容：dist/、openclaw.plugin.json、package.json、README.md 关键坑：只放 dist+manifest 会报 plugin not found，必须带 package.json，OpenClaw 靠它发现插件入口。 配置：C:\Users\admin.openclaw\openclaw.json 的 plugins 段，加了： allow: ["clawguard-adapter"] entries.clawguard-adapter: {enabled:true, config:{clawguardUrl:"[http://127.0.0.1:8000](http://127.0.0.1:8000/)", failMode:"closed", protectedTools:[...]}, hooks:{allowConversationAccess:true}} 关键坑：不加 hooks.allowConversationAccess=true，before_agent_run 和 agent_end 会被阻止（日志报 typed hook blocked），输入检查失效。 配置修改前已备份：openclaw.json.bak-20260830-181454 插件覆盖四阶段：before_agent_run→/v1/input/check；before_tool_call→/v1/tool/pre_check；tool_result_persist→/v1/content/check；message_sending→/v1/output/check。 验证：openclaw plugins list 显示 clawguard-adapter enabled；启动日志 Clawguard API is healthy。
文件：E:\tiaozhanbei\MAC\ClawguardV2.1\clawguard\modules\access_control\risk_link.py
- AuditRiskMonitor：读审计 JSONL，按 user/session/时间窗(默认300秒)算风险评分，识别频发试探（block_count 达到阈值判定 probe_likely）。
- DynamicLinePolicy：按风险评分算 penalty(0-2)，动态提高资源所需等级，升级后本应拦截的请求转 human_review 二次审批。 auth_gateway.py 的 check_reason/check_v4 增加了向后兼容的 penalty 参数（默认0，不影响原调用）。 access_control_adapter.py 集成联动：原始判定→审计风险评估→防线升级重判→human_review。审计不可用时 fail-open；可用环境变量 CLAWGUARD_AC_RISK_LINK=0 关闭。 注意：测试时如果同一个用户反复触发 block，审计里会累积事件导致风险评分升高，进而触发防线升级（会误伤后续测试）。测完要清审计文件：E:\tiaozhanbei\MAC\ClawguardV2.1\runtime\audit\audit-events.jsonl（该目录不提交 git）。
- 全量测试：cd /e/tiaozhanbei/MAC/ClawguardV2.1 && py -m pytest tests/ -q（241 passed）
- 访问控制测试：py tests/test_access_control.py（脚本式测试，32 用例）
- 审计联动测试：py -m pytest tests/test_access_control_risk_link.py -v（19 用例）
- 演示脚本：scripts/demo_risk_link.py（启动 Clawguard 后跑，演示访问控制+审计联动） 测试文件都在 E:\tiaozhanbei\MAC\ClawguardV2.1\tests\。
...
3. 确认插件 failMode（当前 closed）在演示时 Clawguard 服务稳定。
