...
**Argus** 是 OpenClaw 智能体框架的安全插件，基于 AOP 切面模式在 Agent Loop 的关键生命周期挂载访问控制逻辑。
...
### 2.1 宏观视角：Argus 在 OpenClaw 中的位置
...
Argus 不修改 OpenClaw 源码，通过框架暴露的三个生命周期钩子实现无侵入挂载。
...
当同一 Trace 内连续拦截次数 ≥ 3，Argus 通过带内信道注入 `RECUSE/0.1` 信号，通知合规 Agent 主动终止 Trace。
...
flowchart TD
    subgraph 入口["入口"]
        ...
        PACKAGE_JSON["package.json<br/>argus-mac v0.2.0"]
    end
