...
# 访问控制模式: rbac | mac | hybrid
ACCESS_CONTROL_MODE = os.getenv("CLAWGUARD_MODE", "rbac")
...
class SecurityLabel:
    ...
    Attributes:
        level: 1-4, 对应 public/internal/secret/top_secret
        categories: 类别集合，用于 compartment 隔离
    """
    level: int = 1
...
class _TextBackend(_RuleBackend):
    ...
    def _split_line(line: str) -> list[str]:
        # 去掉每段的行内注释（# 前需有空白，避免误伤含 # 的路径），再去除首尾空白
...
class _SQLiteBackend(_RuleBackend):
    ...
    def _init_db(self) -> None:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS resources (
                ...
                inherit_mode TEXT DEFAULT 'flat'
            )
        """)
        conn.commit()
        conn.close()
    ...
    def load_users(self) -> dict[str, dict]:
        ...
        for row in cursor.fetchall():
            ...
            users[uid] = {
                "level": level,
                "specials": [s.strip() for s in specials.split(",")] if specials else []
            }
        conn.close()
        return users
    ...
    def save_users(self, users: dict[str, dict]) -> None:
        ...
        for uid, data in users.items():
            specials = ",".join(data["specials"]) if data.get("specials") else ""
            cursor.execute(
                "INSERT INTO users (user_id, level, specials) VALUES (?, ?, ?)",
                (uid, data["level"], specials)
            )
        conn.commit()
        conn.close()
    ...
    def save_resources(self, resources: list[tuple]) -> None:
        ...
        for pattern, required, inherit_mode in resources:
            cursor.execute(
                "INSERT INTO resources (pattern, required_level, inherit_mode) VALUES (?, ?, ?)",
                (pattern, required, inherit_mode)
            )
        conn.commit()
        conn.close()
...
class RuleStore:
    ...
    def add_user_rule(self, user_id: str, level: str, specials: list[str] = None) -> dict:
        ...
        with self._lock:
            ...
            self.users[user_id] = {
                ...
                "specials": [s.strip() for s in (specials or []) if s.strip()]
            }
            self._backend.save_users(self.users)
    ...
    def add_resource_rule(self, pattern: str, required_level: str, inherit_mode: str = "flat") -> dict:
        """添加资源规则"""
        with self._lock:
            normalized = _normalize_level(required_level)
            # 去重：先删除旧的
            self.resources = [(p, r, i) for p, r, i in self.resources if p != pattern]
            self.resources.append((pattern, normalized, inherit_mode))
...
def _is_namespace_pattern(pattern: str) -> bool:
    """判断是否为 tool:/db: 命名空间规则；Windows 盘符路径（如 C:\）不算。"""
    if ":" not in pattern:
        return False
    if len(pattern) >= 2 and pattern[1] == ":" and pattern[0].isalpha():
        return False  # 盘符路径，如 C:\Windows\*
    return True
...
def _match_pattern(pattern: str, value: str) -> bool:
    ...
    判断 value 是否命中 pattern。
      - 含通配符 * ? 用 glob 匹配
      - 含 tool:/db: 命名空间按 前缀/精确 匹配
      - 路径按精确或分隔符边界前缀匹配（避免 C:\\Users 误中 C:\\Users2）
      - 其余按子串匹配（兜底）
    """
    if not value:
        return False
...
def _resource_required_level_tree(path: str = "", tool: str = "", database: str = "") -> int:
    ...
    best_spec = _pattern_specificity("")
    ...
    for pattern, level, inherit_mode in _store.resources:
        for value in candidates:
            ...
            # 计算匹配深度（路径层级数）
            # 命名空间规则（tool:/db:）加上基础深度确保恒高于路径规则；
            # Windows 盘符路径（如 C:\）虽含冒号但按路径计深度。
            if _is_namespace_pattern(pattern):
                # tool:xxx 深度 = 100 + 冒号数量
                # tool:* 深度 = 100 + 1 = 101
                # tool:execute_bash 深度 = 100 + 1 = 101
            ...
            if current_depth > best_depth:
                ...
                if is_override != best_is_override:
                    better = is_override          # override 压过同深度 inherit
                else:
                    better = spec > best_spec      # 同模式类型时更具体者胜

            if not better:
                continue
            best_level = level
            best_depth = current_depth
            best_is_override = is_override
            best_spec = spec
...
def _resource_match_tree(value: str) -> tuple[str, int]:
    """对单个资源值做树状匹配，返回 (最优命中的 pattern, 所需等级)。
    判定规则与 _resource_required_level_tree 完全一致；供 MAC 分支使用，
    避免 MAC 用平铺首匹配导致与 RBAC 结果不一致。"""
...
def _check_mac(user_id: str, path: str = "", tool: str = "", database: str = "") -> bool:
    ...
    规则：
    - 简单安全规则（下读）：用户标签 dominates 资源标签 → 可读
    - 写操作需要 *-属性规则（上写）：用户等级 <= 资源等级
    """
    user_label = _store.get_mac_label(user_id)
    if user_label is None:
        # 未设置 MAC 标签，使用 RBAC 等级转换
    ...
    for value in candidates:
        # 用树状匹配解析资源标签（与 RBAC 判定一致，避免平铺首匹配漏掉更深规则）
...
def _check_rbac(user_id: str, path: str = "", tool: str = "", database: str = "") -> bool:
    """RBAC 基于角色的访问控制（v2 原有逻辑）。"""
    # 1. 请求必须有资源目标
    ...
    # 4. 等级比对（使用树状匹配）
    required = _resource_required_level_tree(path, tool, database)
...
def check(user_id: str, path: str = "", tool: str = "", database: str = "") -> bool:
    ...
    根据 ACCESS_CONTROL_MODE 环境变量选择模式：
      - "rbac":   基于角色的访问控制（默认）
      - "mac":    强制访问控制
      - "hybrid": 双重校验（RBAC && MAC）
    """
...
def check_reason(user_id: str, path: str = "", tool: str = "", database: str = "") -> tuple[bool, str]:
    ...
    if ACCESS_CONTROL_MODE == "mac":
        ...
        if not ok:
            return False, f"block: MAC 安全标签校验失败"
        return True, "allow: MAC 安全标签校验通过"
    elif ACCESS_CONTROL_MODE == "hybrid":
        rbac_ok = _check_rbac(user_id, path, tool, database)
