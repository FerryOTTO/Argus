...
v3 的核心入口仍是 `check()`，但在内部按 `ARGUS_MODE` 环境变量分派到三种访问控制模型：
...
def check(user_id, path="", tool="", database="") -> bool:
    """返回 True=放行, False=拦截。按 ARGUS_MODE 分派。"""
...
`ARGUS_BLOCK_UNKNOWN_USERS=true` 时，不在规则表中的用户一律拦截；默认 `false` 时按 `public(1)` 处理。
...
```bash
ARGUS_MODE=rbac    # 基于角色（默认）
ARGUS_MODE=mac     # 强制访问控制
ARGUS_MODE=hybrid  # 双重校验
```
