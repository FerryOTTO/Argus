...
def check_v4(user_id: str, path: str = "", tool: str = "", database: str = "") -> dict:
    """v4 统一接口入口。调用 check_reason() 返回字典，供 Adapter 使用。"""
    allowed, reason = check_reason(user_id, path, tool, database)
    return {
        "allowed": allowed,
        "reason": reason,
        "user_id": user_id,
