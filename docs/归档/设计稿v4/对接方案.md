...
3. 使用一个统一 FastAPI 服务作为 Clawguard 总入口。
4. OpenClaw 侧的 Plugin 或源码修改只调用 Clawguard，不再各自直接维护安全判断逻辑。
...
|---|---|---|---|
| OpenGuard | FastAPI、JWT、SQLite、Bridge、Web 页面 | 用户入口和身份来源 | 保持独立运行，将 `user_id`、`session_id` 传入 OpenClaw/Clawguard |
...
| 工具安全模块 | Trusted Tool Policy、工具风险判断、审批、SMCP 思路 | 工具调用之前 | OpenClaw Plugin 调 Clawguard `/v1/tool/pre_check` |
...
2. **Clawguard 主服务**：负责统一调用输入输出、访问控制、工具安全、检索安全和审计模块。
...
graph LR
    ...
    S --> OA
```
## 3.1 为什么只建立一个 Clawguard FastAPI
...
- 能直接导入的 Python 模块：在 Clawguard 内通过函数调用；
...
建议直接放入 `clawguard/common/models.py`。
...
```python
from clawguard.modules.access_control.original.auth_gateway import check
...
沙箱继续通过 OpenClaw MCP 配置调用，不经过普通 Adapter 执行命令。Clawguard 只负责在工具调用前返回是否允许以及是否需要转交沙箱。
...
```text
X-Clawguard-User-Id
X-Clawguard-Session-Id
X-Clawguard-Trace-Id
X-Clawguard-Role
```
...
- 改为只调用 Clawguard 的 `/v1/content/check`；
- 原来的三个 SafeGuard 接口由 `retrieval_guard_adapter.py` 在 Clawguard 内部调用。
...
所有 Plugin 共用一个 `clawguard_client.ts`，不要各写一套请求代码。
...
| 2 | Clawguard FastAPI :8000 | 必须 | 统一安全入口 |
...
```text
[OK] Clawguard API       http://127.0.0.1:8000/health
...
4. Github仓库地址为 https://github.com/WT-ever/ClawguardV2.1