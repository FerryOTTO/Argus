...
- 起因:确认「Argus 有没有新版,有的话同步」
- 核查时间:2026-09-10
- 方法:盘点本机全部 Argus 载体 → 统一换行后做**内容级比对** → git 谱系与时间戳取证
- 结论:**没有更新的上游可拉;真正需要同步的是我们自己内部的一条线——`LLMGate/terminal` 落后于 `Argus`。**
...
| 问题 | 结论 |
|---|---|
| 有没有更新的 Argus 版本? | **没有。** 本机最新的载体(`LLMGate-FULL.zip`,今天 15:11 打的)与我们的 `LLMGate/terminal` **内容零差异**,只是少了模型头——它是我们这条线的**下游产物**,不是新款 |
| 我们的发布包要动吗? | **不用。** `release/v1.0test` 已与最新的 `Argus` 一致 ✅ |
| 那要同步什么? | **`Argus` → `LLMGate/terminal`,共 9 个文件**。B 线仍停在"同步写 users.txt"的**旧架构**,而 A 线已是"users.txt 下线、等级存用户库、执法侧从 auth.db 动态回查"的**新架构** |
...
| 载体 | 身份 | 文件数 | 快照时间 | 判定 |
|---|---|---|---|---|
| `Argus` | git 仓,HEAD `5c022e1` | 290 | 09-10 **09:30–09:37** | **A = 最新** ⭐ |
...
| `LLMGate/terminal` | git 仓(LLMGate)的子目录 | 273 | 09-07 ~ 09-10 03:06 | **B = 落后** ⚠️ |
| `LLMGate-FULL.zip`(Downloads) | 今天 15:11 打包,源自 LLMGate 提交 `7e768de`(15:10) | 264 | 09-10 15:11 | **= B,且少了模型头** |
| `argus-desktop/release*/win-unpacked/Argus` | 桌面端构建产物 | 246 | 09-10 12:55 | 与 A 一致 ✅ |
| `Argus-main` | 0.19 MB 残包 | 95 | 08-05 | 可忽略 |
...
|---|---|---|---|
| `argus/modules/access_control/original/auth_gateway.py` | 830 行 | 741 行 | A 多出 `self.bound` / `_load_bound_user()`(单用户绑定) |
| `argus/api/remote_sync.py` | 961 行 | 977 行 | A:`"users.txt 已下线:只写 access_local.json 的 bound_user"` |
| `argus/api/local_config_routes.py` | 228 行 | 228 行 | A:`pass  # users.txt 已下线:用户规则只保留在服务端` |
...
```
argus/modules/retrieval_guard/models/*          # PIGuard 被忽略
!argus/modules/retrieval_guard/models/.gitkeep
argus/modules/io_guard/original/models/
argus/modules/io_guard/original/model_heads/*/  # io_guard 模型头被忽略
```
...
| 优先级 | 动作 |
|---|---|
| P0 | **同步 A → B**:把上述 9 个文件从 `Argus` 同步到 `LLMGate/terminal`,让 LLMGate-FULL 产出用上新架构(users.txt 下线 + `block_threshold: 0.87`) |
| P0 | **修打包方式**:打包前把 `model_heads/` 与 `models/PIGuard` 显式纳入(或把 `.gitignore` 的这两条改成只忽略缓存),否则成品永远缺模型 |
| P1 | 统一"哪棵树是 master":现在 A/B/C 三份并存,C 已对、B 落后,建议以 `Argus` 为准,其余全部由它派生 |
| P2 | 把 `Argus-main`(0.19 MB 残包)标记为废弃,避免误用 |
