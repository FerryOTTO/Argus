...
#   u_secret   | secret     |
#   u_admin    | top_secret | *
# 做法 B:恢复 argus/modules/access_control/original/rules/users.txt 并追加上面 4 行
```
...
| 数据集 | 缺什么 | 现状 | 想拿满分还需要 |
|---|---|---|---|
| `retrieval_guard`(49) | PIGuard 权重(`argus/modules/retrieval_guard/models/PIGuard` 目录**为空**) | 脚本自动检测后只跑 A+C 层,依赖 B 层的 14 条单列「未覆盖」,不崩 | 放权重进去(装了 torch 的机器),三层自动全开,14 条即可判定 |
...
- 用法见包内 `说明.md`;单独跑:`python run_all.py --repo "<Argus 路径>"`。
